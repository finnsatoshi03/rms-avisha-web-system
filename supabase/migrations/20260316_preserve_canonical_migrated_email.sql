...
begin
  ...
  set
    email = case
      when public.users.migrated_to is null
       and coalesce(public.users.deleted, false) = false
       and public.users.migrated_email is not null
      then public.users.migrated_email
      else excluded.email
    end,
...
update public.users
set email = migrated_email
where migrated_to is null
  and deleted = false
  and migrated_email is not null
  and email is distinct from migrated_email;
